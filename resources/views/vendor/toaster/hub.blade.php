    ...
    <template x-for="toast in toasts" :key="toast.id">
        ...
        >
            <i x-text="toast.message"
               class="inline-block select-none not-italic px-6 py-3 rounded shadow-lg text-xl w-full {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
               :class="toast.select({ error: 'bg-primary', info: 'bg-gray-200', success: 'bg-green-600', warning: 'bg-orange-500' })"
            >
